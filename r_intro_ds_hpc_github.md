...
## Session 1: Introduction, M3 WebPortal, R Markdown, and GitHub
...
| R Markdown | Write and run R code in a notebook-style document |
...
This bootcamp could be taught by having everyone install R on their own laptop.
...
RStudio or RStudio with R support
...
> Note: If you skip this step, you may not be able to launch your R working environment later.
...
3. Select the RStudio or RStudio app used for this bootcamp
4. Launch a session
5. Open the R working environment
...
☐ Launched the R working environment
...
You have successfully connected to M3 and executed your first R code.

---

## Next Up

Now that everyone is working in the same environment, we'll learn how R Markdown notebooks actually work.

# R Markdown Survival Skills

Throughout this R version of the bootcamp, almost all of our work will happen inside R Markdown notebooks.

R Markdown notebooks allow us to combine:
...
This makes them useful for data science, reproducible research, and project documentation.

---

## Chunks

R Markdown notebooks are made up of chunks.

There are two primary types of chunks:

### Markdown Sections

Markdown chunks are used for:
...
When we execute a code chunk, the result is displayed directly below the cell.
...
Try executing the following chunks.
...
A very important concept in R Markdown notebooks is **state**.
...
Where did R get the value of `favorite_number`?
...
Behind every R Markdown notebook is an R session.

The R session is responsible for:

- Running code
- Storing variables
- Managing memory

When you run a code chunk, you are sending instructions to the R session.
...
But the R session forgot everything stored in memory.

This is one of the most common sources of confusion for new R users.
...
Because the notebook text and the R session memory are not necessarily synchronized.
...
R Markdown notebooks save automatically.
...
☐ Understand what the R session does
...
Inside RStudio, open a terminal from the Launcher page.
...
https://github.com/SMUREU/r_bootcamp
...
```bash
git clone git@github.com:YOUR_USERNAME/r_bootcamp.git
```
...
This afternoon we begin working with real datasets in R.